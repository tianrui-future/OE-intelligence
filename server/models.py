"""
OE Intelligence Hub - Pydantic Data Models
...
class SourceMetadata(BaseModel):
    ...
    credibility: Literal["official", "authoritative", "unverified"] = Field(
        description="可信度: official=官方公告, authoritative=权威媒体, unverified=待核实"
    )
...
class OrgNode(BaseModel):
    ...
    change_type: Optional[Literal["new", "removed", "adjusted", "unchanged"]] = Field(
        None, description="变更类型(用于对比模式)"
    )
...
class TimelineEvent(BaseModel):
    ...
    event_type: Literal["restructure", "appointment", "resignation", "merge", "split", "other"] = Field(
        "other", description="事件类型"
    )
...
class ExecutiveFlow(BaseModel):
    ...
    flow_type: Literal["inflow", "outflow"] = Field(description="流入或流出")
    status: Literal["confirmed", "pending", "rumor"] = Field(description="状态标签")
